// ---------------------------------------------------------------------------
// Table.jsx — canvas renderer for the felt, seats, cards and chips.
//
// Pure drawing: it reads the engine state every render and paints it. No game
// logic lives here. When you bring your own character art, the easiest swap is
// drawAvatar() below — replace the emoji circle with an <img>/sprite.
// ---------------------------------------------------------------------------
...
function draw(ctx, state, bubbles, dealerSay, revealAll) {
  ...
  for (let seat = 0; seat < n; seat++) {
    ...
    const sx = cx + Math.cos(angle) * (rx + 70)
    const sy = cy + Math.sin(angle) * (ry + 78)
...
function drawSeat(ctx, p, x, y, opts) {
  ...
  // Speech bubble (fades after a moment via fresh ts)
  if (bubble && Date.now() - bubble.ts < 4200) {
    drawBubble(ctx, x, y - 132, bubble.text)
  }
}
